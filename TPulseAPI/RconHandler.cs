			    ...
			    || packetstring.Substring(5).StartsWith("getinfo"))
			{
				...
				var infostring =
					string.Format(
						...
						tPulse.ActivePlayers(), Main.maxNetPlayers - tPulse.Config.MaxSlots,
		...
		private static string ExecuteCommand(string text)
		{
			...
			else if (text.StartsWith("playing") || text.StartsWith("/playing"))
			{
				...
				foreach (TPPlayer player in tPulse.Players)
				{
					...
					{
						count++;
						TPPlayer.Server.SendMessage(string.Format("{0} ({1}) [{2}] <{3}>", player.Name, player.IP, player.Group.Name,
						                                          player.UserAccountName));
					}
				}
				TPPlayer.Server.SendMessage(string.Format("{0} players connected.", count));
			}
