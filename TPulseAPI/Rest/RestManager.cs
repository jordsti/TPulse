...
namespace TPulseAPI
{
	public class RestManager
	{
		...
		private object ServerOff(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			TPulse.ServerHandle.StopServer(!GetBool(parameters["nosave"], false), msg);
		...
		{
			return new RestObject() { { "activeusers", string.Join("\t", TPulse.Players.Where(p => null != p && null != p.UserAccountName && p.Active).Select(p => p.UserAccountName)) } };
		}

		private object UserListV2(RestVerbs verbs, IParameterCollection parameters)
		{
			return new RestObject() { { "users", TPulse.Users.GetUsers().Select(p => new Dictionary<string,object>(){
				{"name", p.Name},
				{"id", p.ID},
				{"group", p.Group},
				{"ip", p.Address},
		...
		private object UserUpdateV2(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			if (!string.IsNullOrWhiteSpace(password))
			{
				try
				{
					TPulse.Users.SetUserPassword(user, password);
			...
			if (!string.IsNullOrWhiteSpace(group))
			{
				try
				{
					TPulse.Users.SetUserGroup(user, group);
		...
		private object UserDestroyV2(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			try
			{
				TPulse.Users.RemoveUser((User)ret);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object BanCreate(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			try
			{
				TPulse.Bans.AddBan(ip, name, parameters["reason"], true);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object BanDestroyV2(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			try
			{
				...
				{
					case "ip":
						if (!TPulse.Bans.RemoveBan(ban.IP, false, false, true))
							return RestResponse("Failed to delete ban (already deleted?)");
						break;
					case "name":
						if (!TPulse.Bans.RemoveBan(ban.Name, true, GetBool(parameters["caseinsensitive"], true)))
							return RestResponse("Failed to delete ban (already deleted?)");
						break;
					default:
		...
		private object BanListV2(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			foreach (var ban in TPulse.Bans.GetBans())
			{
				banList.Add(
		...
		private object WorldChangeSaveSettings(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			TPulse.Config.AutoSave = autoSave;
		...
		private object GroupList(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			foreach (Group group in TPulse.Groups)
			{
		...
		private object GroupDestroy(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			try
			{
				TPulse.Groups.DeleteGroup(group.Name, true);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object GroupCreate(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			try
			{
				TPulse.Groups.AddGroup(name, parameters["parent"], parameters["permissions"], parameters["chatcolor"], true);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object GroupUpdate(RestVerbs verbs, IParameterCollection parameters)
		{
			...
			try
			{
				TPulse.Groups.UpdateGroup(group.Name, parent, permissions, chatcolor);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object UserFind(IParameterCollection parameters)
		{
			...
			try
			{
				switch (type)
				{
					...
					case "name":
						type = "name";
						user = TPulse.Users.GetUserByName(name);
						break;
					case "id":
						user = TPulse.Users.GetUserByID(Convert.ToInt32(name));
						break;
					case "ip":
						user = TPulse.Users.GetUserByIP(name);
		...
		private object BanFind(IParameterCollection parameters)
		{
			...
			{
				case "ip":
					ban = TPulse.Bans.GetBanByIp(name);
					break;
				case "name":
					ban = TPulse.Bans.GetBanByName(name, GetBool(parameters["caseinsensitive"], true));
					break;
				default:
					return RestError("Invalid Type: '" + type + "'");
			}
		...
		private object GroupFind(IParameterCollection parameters)
		{
			...
			var group = TPulse.Groups.GetGroupByName(name);
			if (null == group)
