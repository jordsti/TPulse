...
namespace TPulseAPI
{
	/*internal class UpdateManager
	{
		...
		private static Dictionary<string, string> ServerIsOutOfDate()
		{
			using (var client = new WebClient())
			{
				...
				try
				{
					...
					if (TPulse.VersionNum.CompareTo(version) < 0)
						return update;
				}
				catch (Exception e)
				{
		...
		private static void NotifyAdministrators(Dictionary<string, string> update)
		{
			...
			NotifyAdministrator(TPPlayer.Server, changes);
		...
		private static void NotifyAdministrator(TPPlayer player, string[] changes)
		{
