...
namespace ChestControl
{
    internal static class Commands
    {
        ...
        public static void Load()
        {
            ...
            tPulse.Commands.ChatCommands.Add(new Command("protectchest", Set, "cset", "setchest"));
            tPulse.Commands.ChatCommands.Add(new Command("protectchest", UnSet, "cunset", "unsetchest"));
            tPulse.Commands.ChatCommands.Add(new Command("protectchest", CancelSet, "ccset", "ccunset", "cancelsetchest", "cancelunsetchest"));

            tPulse.Commands.ChatCommands.Add(new Command("protectchest", SetRegionChest, "crset", "rchest", "regionsharechest"));
            tPulse.Commands.ChatCommands.Add(new Command("protectchest", SetPublicChest, "cpset", "pchest", "setpublicchest"));
            tPulse.Commands.ChatCommands.Add(new Command("protectchest", SetPasswordChest, "clock", "lockchest", "chestlock") { DoLog = false });
            tPulse.Commands.ChatCommands.Add(new Command("refillchest", SetRefillChest, "crefill", "refillchest", "chestrefill"));

            //everyone can unlock
            tPulse.Commands.ChatCommands.Add(new Command(UnLockChest, "cunlock", "unlockchest", "chestunlock") { DoLog = false });

            //add permissions to db if not exists
            bool perm = tPulse.Groups.groups.Where(@group => @group.Name != "superadmin").Any(@group => group.HasPermission("protectchest"));
            if (!perm)
            {
                ...
                tPulse.Groups.AddPermissions("trustedadmin", permissions);
            }
        }
